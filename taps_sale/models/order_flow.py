...
class OrderFlow(models.Model):
    ...
    pi_type = fields.Selection(related='order_id.pi_type',string='Type', store=True)
    sale_representative = fields.Many2one(related='order_id.sale_representative',string='Sales', store=True)
    date_order = fields.Datetime(related='order_id.date_order',string='Date', store=True)
    user_id = fields.Many2one(related='order_id.user_id',string='CSD', store=True)
    pi_number = fields.Char(related='order_id.pi_number',string='PI Number', store=True)
    pi_date = fields.Date(related='order_id.pi_date', string='PI Date', store=True)
    # amount_total = fields.Monetary(related='order_id.amount_total', string='Order Value')
    currency_id = fields.Many2one("res.currency", string="Currency", store=True)
    partner_id = fields.Many2one(related='order_id.partner_id', string='Customer Name', store=True)
    buyer_name = fields.Many2one(related='order_id.buyer_name', string='Buyer', store=True)
    style_ref = fields.Char(related='order_id.style_ref',string='Style', store=True)
    season = fields.Char(related='order_id.season',string='Season', store=True)
    po_no = fields.Char(related='order_id.po_no',string='PO No.', store=True)
    payment_term_id = fields.Many2one(related='order_id.payment_term_id', string='Payment Term', store=True)
    incoterm = fields.Many2one(related='order_id.incoterm', string='Shipment Term', store=True)
    bank = fields.Many2one(related='order_id.bank', string='Advised Bank', store=True)
    department = fields.Char(related='order_id.department',string='Department', store=True)
