...
class SaleOrderRmc(models.Model):
    ...
    def init(self):
        ...
        select row_number() OVER() AS id,sale_order_lines,oa_id,company_id,partner_id,buyer_name,product_id,product_template_id,fg_categ_type,sizein,sizecm,sizemm,price_subtotal,rmc from (select sl.id as sale_order_lines,so.id as oa_id, so.company_id,so.partner_id,so.buyer_name,sl.sizein,sl.sizecm,sl.sizemm,sl.price_subtotal,sl.rmc,p.id as product_id,pt.id as product_template_id, fg.name as fg_categ_type from sale_order_line as sl inner join sale_order as so on so.id=sl.order_id inner join product_product as p on p.id=sl.product_id inner join product_template as pt on p.product_tmpl_id = pt.id inner join fg_category as fg on pt.fg_categ_type=fg.id where so.sales_type='oa' and sl.product_id is not null and so.state='sale' and sl.price_subtotal > 0 and so.company_id=%s and 'a'=%s) as a)"""
