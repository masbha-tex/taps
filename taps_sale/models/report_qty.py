...
class ReportStockQuantity(models.Model):
    ...
    sale_line_id = fields.Many2one('sale.order.line', string='Sale Order', readonly=True)
...
WITH
    existing_sm (id, product_id, product_qty, date, state, company_id, whs_id, whd_id, ls_usage, ld_usage, sale_line_id) AS (
        SELECT m.id, m.product_id, m.product_qty, m.date, m.state, m.company_id, whs.id, whd.id, ls.usage, ld.usage, m.sale_line_id
        FROM stock_move m
    ...
    all_sm (id, product_id, product_qty, date, state, company_id, whs_id, whd_id, ls_usage, ld_usage, sale_line_id) AS (
...
GROUP BY product_id, state, date, company_id, warehouse_id, sale_line_id
);
"""
