...
class MrpReportWizard(models.TransientModel):
    ...
    def pi_mt_xls_template(self, docids, data=None):
        ...
        for item in items:
            ...
            sheet.write(0, 7, "CTG", column_style)
            sheet.write(0, 8, "B PART", column_style)
            sheet.write(0, 9, "QTY", column_style)
            ...
            for orders in sale_orders:
                ...
                for x,o_data in enumerate(docs):
                    ...
                    mt_buyer = 
                    mt_logo = 
                    mt_sizein = o_data.sizein
                    mt_sizecm = o_data.sizecm
                    if mt_sizein == 'N/A':
                        mt_sizein = ''
                    if mt_sizecm == 'N/A':
                        mt_sizecm = ''
                    mt_finish = o_data.finish
                    mt_ctg = 
                    mt_b_part = 
                    mt_qty = o_data.product_uom_qty
                    mt_pending = mt_qty - sum(m_order.mapped('done_qty'))
                    ...
                    mt_report_data.append(mt_order_data)
                if _range > 0:
                    _range += 2
                _range += len(mt_report_data)
                ...
                for line in mt_report_data:
                    for x in mt_report_data[row_p:]:
                        p_last_one = row
                        if (x[1] == line[1]):
                            product_range += 1
                            row_p += 1
