...
class PpcReportWizard(models.TransientModel):
    ...
    def action_generate_xlsx_report(self):
        ...
        if self.report_type == "sbtm":
            data = {'month_list': self.month_list,'date_from': self.date_from,'date_to': self.date_to}
            return self.sample_btm_data(self, data=data)
    ...
    def stopper_wise_all(self, docids, data=None):
        ...
        for top_bottom in top_bottoms:
            ...
            # all_orders = self.env['operation.details'].browse(m_orders.oa_id.ids)
            all_orders = m_orders.filtered(lambda pr: pr.topbottom == top_bottom).sorted(key=lambda pr: pr.id)        
            
            # sale_orders = self.env['sale.order'].browse(all_orders.order_id.ids).sorted(key=lambda pr: pr.id)
            
            report_name = top_bottom
            # raise UserError((buyers))
    ...
    def sample_btm_data(self, docids, data=None):
        ...
        sheet.write(0, 0, "SA Date", column_style)
        # sheet.write(0, 1, f"PRODUCT: {report_name}", column_style)
        sheet.write(0, 1, "SA Number", column_style)
        sheet.write(0, 2, "Buyer Name", column_style)
        sheet.write(0, 3, "Style Name", column_style)
        sheet.write(0, 4, "Customer Name", column_style)
        sheet.write(0, 5, "Product Type", column_style)
        sheet.write(0, 6, "No. of Shade", column_style)
        sheet.write(0, 7, "ORDER QTY", column_style)
        sheet.write(0, 8, "SA Partial Completion Date", column_style)
        sheet.write(0, 9, "SA Final Completion Date", column_style)
        sheet.write(0, 10, "Days to go", column_style)
        sheet.write(0, 11, "LEAD TIME", column_style)
        sheet.write(0, 12, "Status", column_style)
        sheet.write(0, 13, "Remarks", column_style)
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': '/web/content/?model={}&id={}&field=file_data&filename={}&download=true'.format(self._name, self.id, ('SAMPLE BTM DATA')),
            'target': 'self',
        }
