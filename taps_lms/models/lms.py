...
class Session(models.Model):
    ...
    tools = fields.Text('Training Tools', help='Add content note here...') 
...
class EventWizard(models.TransientModel):
    ...
    def create_event_send(self):
        ...
        event_vals = {
            ...
            'description': body, #and tools.html2plaintext(self.note),
            'user_id': user_id,
        ...
        try:
            ...
            template_ctx = {
                'message': self.env['mail.message'].sudo().new(dict(body=event_vals['description'], record_name=lms_session.display_name)),
                'model_description': self.env['ir.model']._get('lms.session').display_name,
