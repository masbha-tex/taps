...
class CrmBuyerAllocated(models.Model):

    _inherit = 'buyer.allocated'
    ...
    def view_visit(self):
        ...
        return {
            ...
            'view_mode': 'tree',
            'res_model': 'crm.visit',
            'domain': [('user_id', '=', self.marketingperson.id)],
            'context': "{'create': False}"
        }

    def _compute_visit_for_individuals(self):
        for record in self:
            visit = record.env['crm.visit'].sudo().search([('user_id', '=', self.marketingperson.id)])
