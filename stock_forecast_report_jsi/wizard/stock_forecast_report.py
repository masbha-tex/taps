...
class StockForecastReport(models.TransientModel):
    ...
    def print_date_wise_stock_register(self):
        ...
        for categ in products.categ_type:
            ...
            opening_categ_qty=sum(row[2] for row in report_product_data)#sum(report_product_data.mapped('opening_qty'))
            opening_categ_value=sum(row[3] for row in report_product_data)
            #sum(report_product_data.mapped('opening_value'))
            received_categ_qty=sum(row[4] for row in report_product_data)
            #sum(report_product_data.mapped('received_qty'))
            received_categ_value=sum(row[5] for row in report_product_data)
            #sum(report_product_data.mapped('received_value'))
            issued_categ_qty=sum(row[6] for row in report_product_data)#sum(report_product_data.mapped('issued_qty'))
            issued_categ_value=sum(row[7] for row in report_product_data)
            #sum(report_product_data.mapped('issued_value'))
            closing_categ_qty=sum(row[8] for row in report_product_data)#sum(report_product_data.mapped('closing_qty'))
            closing_categ_value=sum(row[9] for row in report_product_data)
        ...
        worksheet.write(6, 0, 'Category', column_product_style)
        worksheet.write(6, 1, 'Product', column_product_style)
        worksheet.write(6, 2, 'Opening Quantity', column_product_style)
        worksheet.write(6, 3, 'Opening Value', column_product_style)
        worksheet.write(6, 4, 'Received Quantity', column_received_style)
        worksheet.write(6, 5, 'Received Value', column_received_style)
        worksheet.write(6, 6, 'Issued Quantity', column_issued_style)
        worksheet.write(6, 7, 'Issued Value', column_issued_style)
        worksheet.write(6, 8, 'Closing Quantity', column_product_style)
        worksheet.write(6, 9, 'Closing Value', column_product_style)
        col = 0
        row=7
